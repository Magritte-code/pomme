...
from test_1D import forward, forward_2_lines, forward_analytic_velo, forward_analytic_velo_and_T, forward_analytic_velo_and_T_2_lines, get_model, line, frequencies, velocities, r_in, v_fac
...
def fit_2_lines(loss, spherical, obs, obs_2, N_epochs=10, lr=1.0e-1, w_rep=1.0, w_rep_2=1.0, w_reg=1.0, w_cnt=1.0):
    ...
    for _ in tqdm(range(N_epochs)):
        ...
        # Run forward model
        img, img_2 = forward_analytic_velo_and_T_2_lines(spherical)
 
        # Compute the reproduction loss
        loss['rep_1'] = w_rep   * torch.nn.functional.mse_loss(img,   obs  )
        loss['rep_2'] = w_rep_2 * torch.nn.functional.mse_loss(img_2, obs_2)
        # Compute the regularisation loss
        loss['reg'] = w_reg * diff_loss(spherical.model_1D['log_CO'])
        # Compute the hydrodynamic loss
        # loss['tmp'] = w_tmp * steady_state_heat_loss(spherical, torch.from_numpy(r))    
...
def reconstruct_2_lines(spherical, obs, obs_2):

    # loss = Loss(['rep', 'reg', 'cnt'])
    loss = Loss(['rep_1', 'rep_2', 'reg', 'cnt'])

    img, img_2, loss = fit_2_lines(loss, spherical, obs, obs_2, N_epochs=3  , lr=1.0e-1, w_rep=1.0e+0, w_rep_2=1.0e+0, w_reg=1.0e-0, w_cnt=1.0e+0)
    loss.renormalise_all()
    loss.reset()
    img, img_2, loss = fit_2_lines(loss, spherical, obs, obs_2, N_epochs=350, lr=1.0e-1, w_rep=1.0e+0, w_rep_2=1.0e+0, w_reg=1.0e-1, w_cnt=1.0e+0)
    img, img_2, loss = fit_2_lines(loss, spherical, obs, obs_2, N_epochs=350, lr=1.0e-1, w_rep=1.0e+2, w_rep_2=1.0e+2, w_reg=1.0e+0, w_cnt=1.0e+1)
    img, img_2, loss = fit_2_lines(loss, spherical, obs, obs_2, N_epochs=350, lr=1.0e-1, w_rep=1.0e+3, w_rep_2=1.0e+3, w_reg=1.0e+0, w_cnt=5.0e+1)
    img, img_2, loss = fit_2_lines(loss, spherical, obs, obs_2, N_epochs=350, lr=1.0e-1, w_rep=1.0e+5, w_rep_2=1.0e+5, w_reg=1.0e+0, w_cnt=1.0e+2)
