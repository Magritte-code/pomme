...
def get_initial_model(from_model, nCO, vel):
    ...
    model_1D['log_velocity'] = np.log(vel) * np.ones(model_1D.shape)
...
def reconstruct(spherical, obs):
    ...
    img = fit(loss, spherical, obs, N_epochs=3  , lr=1.0e-1, w_rep=1.0e+0, w_reg=1.0e-0, w_cnt=1.0e+0)
    loss.renormalise_all()
    loss.reset()
    img = fit(loss, spherical, obs, N_epochs=250, lr=5.0e-1, w_rep=1.0e+0, w_reg=1.0e-2, w_cnt=1.0e+0)
    img = fit(loss, spherical, obs, N_epochs=250, lr=1.0e-1, w_rep=1.0e+1, w_reg=1.0e-2, w_cnt=1.0e+1)
    img = fit(loss, spherical, obs, N_epochs=250, lr=5.0e-2, w_rep=1.0e+2, w_reg=1.0e-2, w_cnt=5.0e+1)
    img = fit(loss, spherical, obs, N_epochs=250, lr=1.0e-2, w_rep=1.0e+3, w_reg=1.0e-2, w_cnt=1.0e+2)
